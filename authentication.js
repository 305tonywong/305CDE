...
let mailOptions = {
    from: '"DO NOT REPLY" <tony305wong@gmail.com>', // sender address
    to: request.payload.email, // list of receivers
    subject: 'Account verification email', // Subject line
    //text: 'Hello world ?', // plain text body
    html: "Dear User,<br>Thank you for signing up with Quit Smoking. In order to verify your account, please click on the link and follow the instructions with your <u>username</u> and <u>password</u>.<br><a href=" + link + ">Click here to verify</a>"
   }
...
//use $saddtoset to avoid adding duplicated items into the fav list
    User.update( {"username": request.auth.credentials.username}, {$addToSet: {"fav":{"book_id":request.params.book_id, "image":request.payload.image, "url":request.payload.url, "title":request.payload.title, "author":request.payload.author}}},
    function (err, data) {
         if (err) return console.error(err);
        return reply(data);
     }
...
//check if user has logged in
    ...
    console.log(request.params.book_id);
		if (request.auth.isAuthenticated) {
    User.update( {"username": request.auth.credentials.username},  {$pull:{"fav":{"book_id":request.params.book_id}}},
     function (err, data) {
         if (err) return console.error(err);
        return reply(data);
     }
